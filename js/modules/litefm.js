...
$(document).ready(function(){
	...
	$("#upload.operations-button").click(function(){
		$('#dialog').html('<div class="status"></div>\
						  <form id="upload" action="home.php?m=litefm&home_id='+home_id+'&type=cleared" method="post" enctype="multipart/form-data">\
				...
				if(typeof data.count !== 'undefined')
				{
					...
					var refresh = setInterval(function(){
						$.each(files_info, function(index, file){
							...
							$.ajax({
									type: "POST",
									url: 'home.php?m=litefm&home_id='+home_id+'&type=cleared&pid='+file['pid']+'&size='+file['size']+'&filename='+file['filename'],
									dataType:'json',
									async: false,
									success: function(data){
