...
function exec_ogp_module()
{
	...
	else
	{
		...
		if( preg_match("/Linux/", $os) AND preg_match("/64/", $os) AND preg_match("/wine/", $os) )
		{
			foreach ( $game_cfgs as $row )
			{
				if ( preg_match("/_linux(32|64)?/i", $row['game_key']) )
				{
					echo "<option value='".$row['home_cfg_id']."'>".$row['game_name'];
					if ( preg_match("/_linux64/i", $row['game_key']) ) echo " (64 bit)";
					echo "</option>\n";
				}
			}
			...
			foreach ( $game_cfgs as $row )
			{
				if ( preg_match("/_win(32|64)?/i", $row['game_key']) )
				{
					echo "<option value='".$row['home_cfg_id']."'>".$row['game_name'];
					if ( preg_match("/_win64/i", $row['game_key']) ) echo " (64 bit)";
					echo "</option>\n";
				}
			}
		}
		...
		elseif( preg_match("/CYGWIN/", $os) AND preg_match("/WOW64|x86_64/", $os))
		{
			foreach ( $game_cfgs as $row )
			{
				if ( preg_match("/_win(32|64)?/i", $row['game_key']) )
		...
		elseif ( $os == "Unknown OS" )
		{
			...
			{
				echo "<option value='".$row['home_cfg_id']."'>".$row['game_name'];
				if ( preg_match("/_(win|linux)64/i", $row['game_key']) ) echo " (64 bit)";
				echo "</option>\n";
			}
		}
		echo "</select>\n</td></tr>";
