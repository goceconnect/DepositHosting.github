...
function getMyFile($url,$destination)
{
	...
	if( ( ini_get('open_basedir') or get_true_boolean(ini_get('safe_mode')) ) == FALSE )
...
function exec_ogp_module() 
{
	...
	{
		$install_nfo = file_get_contents($_POST['remove']."/".$_POST['folder']."/install.nfo");
		$files = explode("\n", $install_nfo);
		unset($files[0]);
		array_pop($files);
	...
	$url = 'http://sourceforge.net/projects/ogpextras/rss?limit=50';
