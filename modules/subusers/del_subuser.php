...
function listAllSubUsers() {
    ...
    $htmlCode = "";
    $subusers = $db->listSubUsersByParent($_SESSION['user_id']);
    ...
    if (is_array($subusers)) {
        ...
        if (count($subusers) > 0) {
            ...
            foreach ($subusers as $subuser) {
                $user_info = $db->getUserById($subuser['user_id']);
                $htmlCode.= '<tr><td>' . $user_info['users_login'] . '</td><td><form method="post"><input type="hidden" name="user_id" value="' . $subuser['user_id'] . '" /><input type="submit" value="Edit" name="editUser" /><input type="submit" value="Delete" name="delUser" /></form></td></tr>';
            }
        } else {
