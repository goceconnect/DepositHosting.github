...
//------------------------------------------------------------------------------------------------------------+
	function lgsl_query_34(&$server, &$lgsl_need, &$lgsl_fp)
	{
		...
		if($murmur->is_online())
		{
			...
			$server['s']['name']		= $status['original']['name'];
			$server['s']['playersmax']	= $status['original']['x_gtmurmur_max_users'];
			$server['s']['map']			= "murmur";
			...
			foreach($users as $user)
			{
				...
				$server['p'][$i]['time']	= $user['onlinesecs'];
				$i++;
			}
			}
...
//------------------------------------------------------------------------------------------------------------+
//------------------------------------------------------------------------------------------------------------+
	function lgsl_query_35(&$server, &$lgsl_need, &$lgsl_fp)
	{
		//http://www.rigsofrods.com/servers
		//RoR Servers List via Curl.
		$serverlist_url = "http://www.rigsofrods.com/servers";
		$ip = $server['b']['ip'];
		$ip = gethostbyname($ip);
		...
		foreach ( $matches[1] as $servers ) 
		{
			...
			{
				//server name
				preg_match_all("((.*)<br/><div style=\"font-size\:0\.8em\;margin-top\:5px\;color\:#666666\;\">)siU", $servers, $hostname);
				$server_name = str_replace('(<span style="color:#00aa00;">ranked</span>)', "", $hostname[1][0]);
				$server_name = str_replace('(<span style="color:#aa0000;">password</span>)', "", $server_name);
				$server['s']['name'] = trim($server_name);
				//players
				preg_match_all("(</div></td><td style=\"vertical-align\:top\;\"><span style=\"color\:#......\;\">(.*)</td><td style=\"vertical-align\:top\;\">)siU", $servers, $playerscnt);
				$pieces = explode('</span> / ', $playerscnt[1][0]);
				$server['s']['players'] = $pieces[0];			
				$server['s']['playersmax'] = str_replace(' <span style="color:#aa0000;">[full]</span>', "", $pieces[1]);
				//map
				preg_match_all("(</td><td style=\"vertical-align:top;\">(.*)</td><td style=\"vertical-align:top;\"><span style=\"color)siU", $servers, $mapname);
				$mapstring = $mapname[1][0];
				$mapstring = trim($mapstring);
				$mapstring = str_replace('<span style="color:#00aa00;">'.$pieces[0].'</span> / '.$pieces[1], "", $mapstring);
				$mapstring = str_replace('<span style="color:#aa0000;">'.$pieces[0].'</span> / '.$pieces[1], "", $mapstring);
				$mapstring = str_replace('</td><td style="vertical-align:top;">', "", $mapstring);
				$mapstring = str_replace('<a href="http://www.rigsofrods.com/repository/index/Search.keywords:', "", $mapstring);
				$mapstring = str_replace('</a>', "", $mapstring);
				$map_pieces = explode('">', $mapstring);
				$mapstring = $map_pieces[0];
				$mapstring = str_replace('.terrn', "", $mapstring);
				preg_match_all("(terrn\">(.*)</a>)siU", $mapstring, $mapresult);
				@$mapname = $mapresult[1][0];
				if(empty($mapname))
					$server['s']['map'] = $mapstring;
				else
					$server['s']['map'] = $mapname;
				//players list
...
//------------------------------------------------------------------------------------------------------------+
	function lgsl_query_37(&$server, &$lgsl_need, &$lgsl_fp)
	{
		...
		$ip = gethostbyname($ip);
		$port = $server['b']['c_port'];
		$serversumary_url = "http://$ip:$port/stats?sid=1";
		$plain_xml = file_get_contents($serversumary_url);
		$xml = ( isset( $plain_xml ) and $plain_xml != "" ) ? new SimpleXMLElement( file_get_contents($serversumary_url) ) : FALSE;
		if ( $xml )
		{
		/* 	echo $xml->CURRENTLISTENERS;echo "<br>";
			echo $xml->PEAKLISTENERS;echo "<br>";
			echo $xml->MAXLISTENERS;echo "<br>";
			echo $xml->UNIQUELISTENERS;echo "<br>";
			echo $xml->AVERAGETIME;echo "<br>";
			echo $xml->SERVERGENRE;echo "<br>";
			echo $xml->SERVERURL;echo "<br>";
			echo $xml->SERVERTITLE;echo "<br>";
			echo $xml->SONGTITLE;echo "<br>";
			echo $xml->NEXTTITLE;echo "<br>";
			echo $xml->STREAMHITS;echo "<br>";
			echo $xml->STREAMSTATUS;echo "<br>";
			echo $xml->STREAMPATH;echo "<br>";
			echo $xml->BITRATE;echo "<br>";
			echo $xml->CONTENT;echo "<br>";
			echo $xml->VERSION;echo "<br>";  */

			$server['s']['name']		= !empty( $xml->SERVERTITLE ) ? $xml->SERVERTITLE." (".$xml->SERVERGENRE.") Next : <b>".$xml->NEXTTITLE."</b>" : "No one is broadcasting.";
...
//------------------------------------------------------------------------------------------------------------+
//------------------------------------------------------------------------------------------------------------+
	function lgsl_query_39(&$server, &$lgsl_need, &$lgsl_fp)
	{
		//IVMP
		$ip = $server['b']['ip'];
		$port = $server['b']['c_port'];
		...
		if(!$q->Query($ip,$port,$errno,$errstr,2))
		{
			return FALSE;
		}
		else
		{
			$server_data = $q->ServerData();
			$server['s']['players'] = $server_data['players'];
			$server['s']['playersmax'] = $server_data['maxplayers'];
			$server['s']['name'] = $server_data['hostname'];
			$server['s']['map'] = "gtaiv";
			...
			if(isset($server['s']['name']) and $server['s']['name'] != "")
			$q->Close();
			return TRUE;
		}
	}
//------------------------------------------------------------------------------------------------------------+
//------------------------------------------------------------------------------------------------------------+ 
	function lgsl_query_40(&$server, &$lgsl_need, &$lgsl_fp)
	{
	//---------------------------------------------------------+
	//  REFERENCE: http://developer.valvesoftware.com/wiki/Server_Queries
		...
		$server['s']['name']		= lgsl_cut_string($buffer);
		$players_string			 = lgsl_cut_string($buffer);
		$players_string			 = str_replace('Players: ','',$players_string);
		list($server['s']['players'],$server['s']['playersmax']) = explode('/',$players_string);
		$server['s']['game']		= lgsl_cut_string($buffer);
		$server['s']['map']		 = str_replace(':','',lgsl_cut_string($buffer));
