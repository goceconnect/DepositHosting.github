	...
	if($ts3_ServerInstance->isOnline() == TRUE)
	{
		...
		if(isset($_POST['new_ts3_port']) AND $server_home['home_id'] == $_POST['home_id'] )
		{
			...
			if ($AddVirtual === TRUE)
			{
				...
				echo "<meta HTTP-EQUIV='REFRESH' content='0; url=?m=gamemanager&p=game_monitor&home_id-mod_id-ip-port=".
					  $server_home['home_id'] . "-". $server_home['mod_id'] . "-" . $server_home['ip'] . "-" . $new_port . "'>";
			}
		...
		if(isset($_POST['del_ts3_port']) && $_POST['del_ts3_port'] == $cfg["voice"] AND $server_home['home_id'] == $_POST['home_id'] )
		{
			if( ! $startup_file_exists )
			{
				...
				if ($DelVirtual === TRUE)
				{
					...
					/* refresh */
					echo "<meta HTTP-EQUIV='REFRESH' content='0; url=?m=gamemanager&p=game_monitor&home_id=" . $server_home['home_id'] . "'>";
				}
